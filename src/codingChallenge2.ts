...
function getNDecimalPlace1(num: number, place: number): number {
  return Math.round(num * 10 ** place) / 10 ** place;
}

function calculateBMI1(person: Person) {
  const result = person.weight / person.height ** 2;
  return getNDecimalPlace1(result, 2);
}

const mark1BMI: ReturnType<typeof calculateBMI1> = calculateBMI1(mark1);
const john1BMI: ReturnType<typeof calculateBMI1> = calculateBMI1(john1);
